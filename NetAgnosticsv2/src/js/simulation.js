class Simulation {
    ...
    updateData(url) {
        this.isRealTime = !url;
        this.userDict={}; this.userReverseDict={};
        this.computeDict={}; this.computeReverseDict={};
        if (!this.isRealTime) {
            ...
            let updatePromise=(_.isString(url)?d3.json(url):url).then(d=>preadjustdata(d)).then((data) => {
                ...
                Object.keys(data.jobs_info).forEach(jID=>{if (!this.userDict[data.jobs_info[jID].user_name] && !this.userReverseDict[data.jobs_info[jID].user_name]){
                    const encoded =  'user'+Object.keys(this.userDict).length;
                    ...
                    data.jobs_info[jID].node_list = data.jobs_info[jID].nodes??data.jobs_info[jID].node_list.map(c=>{
                        let split = c.split('-');
                ...
                Object.keys(data.nodes_info).forEach(k=>{
                    const ip = k.split('.');
    ...
    queryRange(timepoint,step,list){
        ...
        if(this.data&&list.length){
            ...
            list.forEach(c => {
                nodes_info[c] = {};
                Object.keys(self.data.nodes_info[c]).forEach(s => {
           ...
           let timer;
            ...
            function update() {
                if (self.data) {
                    ...
                    const nodes_info = {};
                    Object.keys(self.data.nodes_info).forEach(c => {
                        nodes_info[c] = {};
                        Object.keys(self.data.nodes_info[c]).forEach(s => {
    ...
    requestFromURL(start,end){
       ...
       // todo need method to cancle
        ...
        return d3.json(url).then(function(data){
            ...
            const jobs_info_list = Object.keys(data.jobs_info);
            jobs_info_list.forEach(jID=>{
                if (!self.userDict[data.jobs_info[jID].user_name])
                    self.userDict[data.jobs_info[jID].user_name] = 'user'+Object.keys(self.userDict).length;
                data.jobs_info[jID].user_name = self.userDict[data.jobs_info[jID].user_name];
                data.jobs_info[jID].node_list = data.jobs_info[jID].nodes??data.jobs_info[jID].node_list.map(c=>c.split('-')[0]);
                if(data.jobs_info[jID].start_time>9999999999999)
                {data.jobs_info[jID].start_time = data.jobs_info[jID].start_time/1000000
                    data.jobs_info[jID].submit_time = data.jobs_info[jID].submit_time/1000000
                    if (data.jobs_info[jID].finish_time && data.jobs_info[jID].finish_time>9999999999999)
